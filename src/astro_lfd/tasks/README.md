...
`StreakDetectTask` is selected by setting the `detection_algorithm` 
configurable parameter in `StreakDetectConfig`. New detector implementations 
...
in the `StreakDetectTask` constructor. 
...
`StreakDetectTask` is a pipeline task used to detect streaks in a difference
