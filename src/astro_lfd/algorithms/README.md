...
**KHTDetectTask.** `KHTDetectTask` finds straight linear features (satellite 
streaks, and similar signals) in an `lsst.afw.image.Exposure`.  It reproduces 
the detection stages of `lsst.meas.algorithms.maskStreaks.MaskStreaksTask` --
Canny edge extraction, `lsst.kht` line finding, recursive k-means clustering,
and a Moffat line-profile fit -- but emits its results as a 
`~lsst.afw.table.SourceCatalog` of canonical line segments (via 
`~astro_lfd.table.streakAdapter.StreakAdapter`) instead of a mask plane.
