...
class KHTDetectConfig(pexConfig.Config):
    ...
    use_streak_threshold = pexConfig.Field(
        doc="Use streak detection threshold.",
        dtype=bool,
        default=False,
    )
    streak_nsigma = pexConfig.Field(
        doc="Number of sigma for streak detection threshold.",
        dtype=float,
        default=2.0,
    )
...
class KHTDetectTask(pipeBase.Task):
    ...
    def preprocess(
        ...
        # Edge thin detect or new streak mask
        if self.config.use_streak_threshold:
            sky_noise = None
            ...
            if sky_noise is not None:
                detection_noise = sky_noise / self.config.bin_size
            streak_mask = mi.image.array > (self.config.streak_nsigma * detection_noise)
        else:
            streak_mask = detected_mask
        init_edges = canny(streak_mask.astype(np.float64), use_quantiles=True, sigma=0.1)
