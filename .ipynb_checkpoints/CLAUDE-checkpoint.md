...
- The Jupyter notebooks in this repo (`ADRTStudy.ipynb`, `Detector_ADRT.ipynb`)
  are **scratch / experimentation only**. Do **not** treat them as a definitive
  specification of the API, the intended workflow, or best practice. They may
  contain dead ends, throwaway code, and environment-specific hacks (e.g. Rubin
  Butler / LSST camera access). Mine them for ideas, not for contracts.
...
astro-adrt/
├── CLAUDE.md              # this file
├── ADRTStudy.ipynb        # scratch: exploration of the ADRT output structure
├── Detector_ADRT.ipynb    # scratch: early detector experiments on real data
